...
//  project 5
//
//  Created by Hubert Leszkiewicz on 28/06/2020.
...
class ViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        if allWords.isEmpty {
            ...
            navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .redo, target: self, action: nil)
        }
startgame()
        ...
        if isPossible(word: lowerAnswer) {
            if isOriginal(word: lowerAnswer) {
                if isReal(word: lowerAnswer) {
                    ...
                    errorMessage = "You cant just make them up, ya know!"
                }
            }else{
