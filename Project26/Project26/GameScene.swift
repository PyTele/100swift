...
//  Project26
//
//  Created by Hubert Leszkiewicz on 29/11/2020.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func loadLevel() {
        guard let levelURL = Bundle.main.url(forResource: "level1", withExtension: "txt") else {fatalError("Could not find level1.txt in app bundle.") }
...
//                    load vortex
                    ...
                    node.name = "vortex"
                    node.position = position
                    node.run(SKAction.repeatForever(.rotate(byAngle: .pi, duration: 1)))
...
//                    load finish point
                    ...
                    node.physicsBody?.contactTestBitMask = CollisionTypes.finish.rawValue
                    node.physicsBody?.collisionBitMask = 0
                    node.position = position
                    addChild(node)
                } else if letter == " " {
//                    empty space, do nothing
    ...
    func createPlayer() {
        ...
        player.physicsBody?.collisionBitMask = CollisionTypes.player.rawValue
